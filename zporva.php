...
<head>
    ...
    <style>
        ...
        * {
        ...
        .linkedin-job-apply:hover {
            background-color: var(--primary);
            transform: translateY(-2px);
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);
        }
        
        .linkedin-job-apply i {
            margin-left: 0.5rem;
        }
...
<body>
    ...
    <section id="open-positions" class="section">
        ...
        <p class="section-subtitle">Scopri le opportunità di carriera disponibili nel nostro team</p>
        
        <div class="linkedin-jobs-container">
            <div id="linkedin-jobs-list" class="linkedin-jobs-list">
                <!-- Jobs will be loaded here via JavaScript -->
                <div class="linkedin-loading">
                    <i class="fas fa-spinner fa-spin"></i> Caricamento delle posizioni disponibili...
                </div>
            </div>
        </div>
    </section>
    ...
    <script>
        ...
        // Funzione per caricare gli annunci di lavoro da LinkedIn
        async function loadLinkedInJobs() {
            const jobsContainer = document.getElementById('linkedin-jobs-list');
            
            try {
                // Sostituisci con l'ID della tua azienda su LinkedIn
                const companyId = 'acrisureitalia'; // Modifica questo con il tuo ID LinkedIn
                
                // URL dell'API LinkedIn (questa è una soluzione temporanea - vedi nota sotto)
                // NOTA: LinkedIn non offre più un'API pubblica per gli annunci di lavoro. 
                // Questa è una soluzione alternativa che richiede un backend personalizzato.
                const apiUrl = `https://linkedin-jobs-api.herokuapp.com/jobs?company=${companyId}`;
                
                // In alternativa, puoi usare il widget ufficiale di LinkedIn (vedi nota sotto)
                ...
                if (jobs.length === 0) {
                    jobsContainer.innerHTML = `
                        <div class="linkedin-error">
                            <i class="fas fa-info-circle"></i> Al momento non ci sono posizioni aperte.
                            <p>Visita la nostra <a href="https://www.linkedin.com/company/${companyId}/jobs/" target="_blank">pagina LinkedIn</a> per verificare eventuali nuove opportunità.</p>
                        </div>
                    `;
                    return;
                }
                ...
                jobs.forEach(job => {
                    ...
                    jobCard.className = 'linkedin-job-card';
                    jobCard.innerHTML = `
                        <div class="linkedin-job-header">
                            <h3 class="linkedin-job-title">${job.title}</h3>
                            <div class="linkedin-job-company">${job.company}</div>
                            <div class="linkedin-job-location">
                                <i class="fas fa-map-marker-alt"></i> ${job.location}
                            </div>
                            <p class="linkedin-job-description">${job.description}</p>
                        </div>
                        <div class="linkedin-job-footer">
                            <div class="linkedin-job-date">Pubblicato il ${new Date(job.date).toLocaleDateString()}</div>
                            <a href="${job.applyUrl}" target="_blank" class="linkedin-job-apply">
                                Candidati <i class="fas fa-external-link-alt"></i>
                            </a>
                        </div>
                    `;
                ...
                console.error('Errore nel caricamento degli annunci LinkedIn:', error);
                jobsContainer.innerHTML = `
                    <div class="linkedin-error">
                        <i class="fas fa-exclamation-triangle"></i> Impossibile caricare le posizioni aperte.
                        <p>Visita la nostra <a href="https://www.linkedin.com/company/acrisureitalia/jobs/" target="_blank">pagina LinkedIn</a> per vedere le opportunità disponibili.</p>
                        <button onclick="loadLinkedInJobs()" class="linkedin-refresh-btn">
        ...
        // NOTA IMPORTANTE: 
        // LinkedIn non offre più un'API pubblica per gli annunci di lavoro.
        // Le opzioni disponibili sono:
        // 1. Usare il widget ufficiale di LinkedIn (vedi https://business.linkedin.com/talent-solutions/job-embeds)
        // 2. Implementare un backend personalizzato che faccia scraping dei lavori (richiede autorizzazione)
        // 3. Usare un servizio di terze parti che offre API per LinkedIn
        
        // Soluzione alternativa: incorporare il feed LinkedIn direttamente
        function embedLinkedInJobs() {
            const jobsContainer = document.getElementById('linkedin-jobs-list');
            jobsContainer.innerHTML = `
                <div style="width: 100%; height: 600px; overflow: hidden;">
                    <iframe 
                        src="https://www.linkedin.com/embed/feed/update/urn:li:share:YOUR_SHARE_ID" 
                        width="100%" 
                        height="100%" 
                        frameborder="0" 
                        allowfullscreen=""
                        title="Posizioni aperte su LinkedIn">
                    </iframe>
                </div>
                <div style="text-align: center; margin-top: 1rem;">
                    <a href="https://www.linkedin.com/company/acrisureitalia/jobs/" target="_blank" class="btn">
                        Vedi tutte le posizioni su LinkedIn <i class="fab fa-linkedin"></i>
                    </a>
                </div>
            `;
        }
        ...
        window.addEventListener('load', () => {
            ...
            // 1. loadLinkedInJobs(); // Usa l'API (richiede backend)
            // 2. embedLinkedInJobs(); // Usa il widget incorporato
            
            // Per ora usiamo il widget incorporato
            embedLinkedInJobs();
        });
